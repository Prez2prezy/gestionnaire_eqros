...
# FIX (bug n°5) : get_max_membres est importé de services.py (SOURCE UNIQUE).
# L'ancienne copie locale divergeait de celle de view_paroisse ("notre dame de
# l'assomption" vs "notre dame" + "assomption") : la même équipe avait un quota
# différent selon le compte connecté. Le `return 12` inatteignable est supprimé.
...
def show_equipe():
    ...
    # CORRECTION MÉMOIRE (conservée) : nettoyage des formulaires fantômes au changement de menu
    ...
    if menu == "👥 Mon équipe":
        ...
        if eq:
            ...
            st.markdown(f'<div class="custom-info-box"><b>Responsable :</b> {eq[0] or ""}<br><b>Bureau :</b> {eq[1] or ""}<br><b>Effectif :</b> {nb}/{max_membres} membres</div>', unsafe_allow_html=True)
            with st.expander("✏️ Modifier les informations"):
                # FIX : st.form (l'ancien bouton nu faisait perdre le focus à chaque frappe)
                with st.form("form_update_equipe"):
                    ...
                    if st.form_submit_button("💾 Enregistrer les modifications", width="stretch"):
                        if nouveau_resp:
                            ...
                            commit_and_sync()
                            st.session_state["flash_success"] = "Informations mises à jour ! ✅"  # FIX : flash mémorisé
                            st.rerun()
                        else:
        ...
        # FIX garde : si la paroisse a été réinitialisée entre-temps, l'ancien
        # fetchone()[0] sans garde provoquait un TypeError
        paroisse_row = c.execute("SELECT p.nom FROM paroisses p JOIN equipes e ON p.id = e.paroisse_id WHERE e.id=?", (eid,)).fetchone()
        paroisse_nom = paroisse_row[0] if paroisse_row else "Paroisse inconnue"

        nb = c.execute("SELECT COUNT(*) FROM membres WHERE equipe_id=? AND statut=?", (eid, 'actif')).fetchone()[0]
        st.info(f"{nb}/{max_membres} membres")

        # FIX : le bouton reste cliquable — la vraie garde quota est À LA SOUMISSION
        # (l'ancien test d'affichage laissait deux sessions simultanées dépasser le quota)
        ...
        if st.session_state.get('open_form_eq') == "ajout":
            with st.container():
                ...
                with st.form("form_ajout_eq"):
                    ...
                    with col1:
                        if st.form_submit_button("❌ Annuler", width="stretch"): st.session_state['open_form_eq'] = None; st.rerun()
                    with col2:
                        if st.form_submit_button("✅ Ajouter", width="stretch"):
                            # FIX (bug n°1 - CRASH VISUEL) : l'ancien `if bouton and nom and prenom: ... else: st.error(...)`
                            # affichait l'erreur EN PERMANENCE (le else s'exécutait à chaque rerun
                            # où le bouton n'était pas cliqué). Structure if/else propre désormais.
                            if not nom or not prenom:
                                st.error("Le nom et le prénom sont requis.")
                            elif c.execute("SELECT COUNT(*) FROM membres WHERE equipe_id=? AND statut='actif'", (eid,)).fetchone()[0] >= max_membres:
                                st.error(f"❌ L'équipe est complète ({max_membres} membres max).")
                            elif c.execute("SELECT id FROM membres WHERE nom=? AND prenom=? AND date_naissance=? AND statut=?",
                                           (nom.strip(), prenom.strip(), naissance.isoformat(), 'actif')).fetchone():
                                ...
                                if photo:
                                    url_photo = sauvegarder_photo(photo, matloc)
                                    if url_photo:
                                        c.execute("UPDATE membres SET photo_path=? WHERE matloc=?", (url_photo, matloc))
                                commit_and_sync()
                                st.session_state['open_form_eq'] = None
                                # FIX (bug n°3) : le MatLoc est le sésame du membre (page de
                                # réponse + espace spirituel) — l'ancien st.success + rerun
                                # l'effaçait AVANT que le responsable puisse le noter !
                                st.session_state['new_membre_info'] = {'mat': matloc, 'nom': nom, 'prenom': prenom}
                                st.rerun()

        # FIX (bug n°3) : affichage PERSISTANT du MatLoc du nouveau membre
        if st.session_state.get('new_membre_info'):
            ...
            st.success(f"✅ {info['nom']} {info['prenom']} ajouté(e) !")
            ...
            if st.button("OK, j'ai noté le MatLoc", key="ok_new_membre_eq"):
                ...
                if st.session_state.get('open_form_eq') == f"mod_{id_m}":
                        ...
                        with st.form(f"form_mod_eq_{id_m}"):
                            ...
                            with col2:
                                if st.form_submit_button("💾 Enregistrer", width="stretch"):
                                    ...
                                    if new_photo:
                                        nouvelle_url = sauvegarder_photo(new_photo, matloc)
                                        if nouvelle_url:
                                            supprimer_photo(m_data[4])
                                            c.execute("UPDATE membres SET photo_path=? WHERE id=?", (nouvelle_url, id_m))
                                    commit_and_sync(); st.session_state['open_form_eq'] = None
                                    st.session_state["flash_success"] = "Membre modifié. ✅"
                                    st.rerun()
                    ...
                    with st.form(f"form_arch_eq_{id_m}"):
                        ...
                        with col1:
                            if st.form_submit_button("❌ Annuler", width="stretch"): st.session_state['open_form_eq'] = None; st.rerun()
                        with col2:
                            if st.form_submit_button("✅ Archiver", width="stretch"):
                                # FIX (bug n°4) : garde équipe pleine RECOMPTÉE à la soumission
                                equipe_pleine = (situation == "Transféré" and equipe_destination and
                                ...
                                if annee_fin_arch <= annee_debut_arch:
                                ...
                                elif situation == "Transféré" and not equipe_destination:
                                    st.error("Choisissez une équipe.")
                                elif equipe_pleine:
                                    st.error("❌ Transfert impossible : l'équipe d'accueil est pleine.")
                                elif situation == "Transféré":
                                    # FIX (bug n°2) : l'ancien code insérait l'ENTIER
                                    # `annee_debut_arch` dans date_debut → corrompu
                                    # (durée 0 an, tri faux). Convention unique 1er sept.
                                    # FIX aussi : un seul UPDATE (l'ancien double
                                    # archive/active était inutile)
                                    c.execute("UPDATE membres SET equipe_id=? WHERE id=?", (equipe_destination, id_m))
                                    c.execute('''INSERT INTO archives (membre_id, situation, date_debut, date_fin, commentaire, auteur_id, auteur_nom, auteur_role, paroisse_id, equipe_id) VALUES (?, 'Transféré', ?, ?, ?, ?, ?, ?, ?, ?)''',
                                              (id_m, date(annee_debut_arch, 9, 1), date(annee_fin_arch, 9, 1), f"Transféré vers {dest_nom}", st.session_state.get('user_id'), st.session_state.get('username'), 'equipe', pid, eid))
                                    commit_and_sync(); st.session_state['open_form_eq'] = None
                                    st.session_state["flash_success"] = f"Transféré vers {dest_nom} ! ✅"
                                    st.rerun()
                                else:
                                    archiver_membre(id_m, situation, annee_debut_arch, annee_fin_arch, commentaire, st.session_state.get('user_id'), st.session_state.get('username'), 'equipe', pid, eid)
                                    st.session_state['open_form_eq'] = None
        ...
        if 'delete_membre_id' in st.session_state:
            ...
            if m_del:
                st.warning(f"⚠️ Supprimer définitivement {m_del[0]} {m_del[1]} ? Cette action efface aussi ses abonnements, présences et archives.")
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("✅ Oui, supprimer"):
                        if m_del[2]: supprimer_photo(m_del[2])
                        # FIX : purge COMPLÈTE — l'ancien code laissait ses
                        # archives en orphelines (invisibles mais polluantes)
                        c.execute("DELETE FROM archives WHERE membre_id=?", (del_id,))
                        ...
                        commit_and_sync()
                        st.session_state.pop('delete_membre_id', None)
                        st.session_state["flash_success"] = "Membre supprimé."
                        st.rerun()
                with col2:
                    if st.button("❌ Annuler"): st.session_state.pop('delete_membre_id', None); st.rerun()

        if membres:
            st.markdown("---\n### 📥 Export des données")
            df_export = pd.DataFrame([(m[1], m[8] or "-", m[2], m[3], m[7] or "-", m[4], m[6]) for m in membres], columns=["MatLoc", "Matricule", "Nom", "Prénom", "N° méditation", "WhatsApp", "Date adhésion"])
            output = io.BytesIO()
            try:
                ...
                output.seek(0)
                # FIX : clé stable (l'ancienne incluait date.today() → changeait chaque jour, inutile)
                st.download_button("📥 Exporter les membres (Excel)", data=output,
                                   file_name=f"membres_{nom_equipe}_{paroisse_nom}_{date.today()}.xlsx",
                                   mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"export_membres_eq_{eid}")
        ...
        for m in c.execute("SELECT id, nom, prenom, matricule FROM membres WHERE equipe_id=? AND statut='actif'", (eid,)).fetchall():
            ...
            if deja:
                ...
                with col_info: st.info(f"{m[1]} {m[2]} ({m[3] or '—'}) – ✅ {type_affiche} ({abo_info[1] or 0} FCFA)")
                with col_btn:
                    if st.button("✏️", key=f"mod_abo_{m[0]}_{annee}"): st.session_state['modif_abo_id'] = m[0]; st.rerun()
            else:
                with st.expander(f"{m[1]} {m[2]} ({m[3] or '—'}) – ❌ Non enregistré"):
                    type_abo, montant = widget_type_abonnement("eq", m[0], annee)
                    if st.button("Enregistrer", key=f"btn_eq_{m[0]}_{annee}"):
                        enregistrer_abonnement(m[0], annee, montant, type_abo)
                        st.session_state["flash_success"] = "Enregistré ! ✅"  # FIX : flash mémorisé
                        st.rerun()
                ...
                with st.form(f"modif_abo_form_{mod_id}"):
                    ...
                    with col1:
                        if st.form_submit_button("💾 Mettre à jour", width="stretch"):
                            ...
                            enregistrer_abonnement(mod_id, annee, new_montant, type_str)
                            st.session_state.pop('modif_abo_id', None)
                            st.session_state["flash_success"] = "Modifié. ✅"
                            st.rerun()
                    with col2:
                        if st.form_submit_button("🗑️ Supprimer", width="stretch"):
                            c.execute("DELETE FROM abonnements WHERE membre_id=? AND annee_debut=?", (mod_id, annee)); commit_and_sync()
                            st.session_state.pop('modif_abo_id', None)
                            st.session_state["flash_warning"] = "Abonnement supprimé."
                            st.rerun()
                    with col3:
                        if st.form_submit_button("❌ Annuler", width="stretch"): st.session_state.pop('modif_abo_id', None); st.rerun()
        ...
        with tab_liste[0]:
            ...
            if abonnes:
                data = [{"Matricule": a[0] or "—", "Nom": a[1], "Prénom": a[2], "Date paiement": a[3], "Montant": f"{a[4] or 0} FCFA"} for a in abonnes]
                df = pd.DataFrame(data); st.dataframe(df, hide_index=True, width="stretch")
        ...
        with tab_liste[1]:
            ...
            if reabonnes:
                data = [{"Matricule": r[0] or "—", "Nom": r[1], "Prénom": r[2], "Date paiement": r[3], "Montant": f"{r[4] or 0} FCFA"} for r in reabonnes]
                df = pd.DataFrame(data); st.dataframe(df, hide_index=True, width="stretch")
        ...
        with tab_liste[2]:
            ...
            if non_inscrits:
                data = [{"Matricule": n[0] or "—", "Nom": n[1], "Prénom": n[2]} for n in non_inscrits]
                df = pd.DataFrame(data); st.dataframe(df, hide_index=True, width="stretch")
        ...
        with tab_avenir:
            ajouter_evenement_agenda(equipe_id=eid, auteur_nom=st.session_state.get('username'))
            st.markdown("---")
                ...
                with st.form("archive_membre"):
                    ...
                    if situation == "Transféré":
                        ...
                        if equipes_dispo:
                            ...
                            equipe_destination = dest_dict[dest_nom]
                            if c.execute("SELECT COUNT(*) FROM membres WHERE equipe_id=? AND statut='actif'", (equipe_destination,)).fetchone()[0] >= get_max_membres(equipe_destination):
                                st.error("⚠️ Impossible : équipe pleine.")
                        else:
                            st.warning("Aucune autre équipe.")
                    if st.form_submit_button("Archiver", width="stretch"):
                        # FIX (bug n°4) : garde équipe pleine RECOMPTÉE à la soumission
                        ...
                        if annee_fin_arch <= annee_debut_arch:
                            ...
                            c.execute("UPDATE membres SET equipe_id=? WHERE id=?", (equipe_destination, membre_choisi[0]))
                            c.execute('''INSERT INTO archives (membre_id, situation, date_debut, date_fin, commentaire, auteur_id, auteur_nom, auteur_role, paroisse_id, equipe_id) VALUES (?, 'Transféré', ?, ?, ?, ?, ?, ?, ?, ?)''',
                                      (membre_choisi[0], date(annee_debut_arch, 9, 1), date(annee_fin_arch, 9, 1), f"Transféré vers {dest_nom}", st.session_state.get('user_id'), st.session_state.get('username'), 'equipe', pid, eid))
                            commit_and_sync()
                            st.session_state["flash_success"] = f"Transféré vers {dest_nom} ! ✅"
                            st.rerun()
                        else:
                            archiver_membre(membre_choisi[0], situation, annee_debut_arch, annee_fin_arch, commentaire, st.session_state.get('user_id'), st.session_state.get('username'), 'equipe', pid, eid)
                            st.session_state["flash_success"] = f"✅ {membre_choisi[1]} {membre_choisi[2]} archivé(e)."
                            st.rerun()
        ...
        if archives_equipe:
            for arch in archives_equipe:
                ...
                with st.expander(f"{nom} {prenom} ({matloc}) – {afficher_situation(situation)} – {duree} an(s) - Sept {d1.year if d1 else '?'} – Août {d2.year if d2 else '?'}"):
                        ...
                        with btn1:
                            if st.form_submit_button("💾 Mettre à jour", width="stretch"):
                                if new_af <= new_ad: st.error("Année invalide.")
                                    ...
                                    commit_and_sync()
                                    st.session_state["flash_success"] = "Archive modifiée. ✅"
                                    st.rerun()
                        with btn2:
                            if new_situation in ("Déplacé", "Radié"):
                                # FIX : se base sur la situation ACTUELLE du selectbox
                                # (l'ancien testait la situation d'origine de la base)
                                if st.form_submit_button("🔄 Réintégrer", width="stretch"):
                                    c.execute("UPDATE membres SET statut='actif' WHERE id=?", (membre_id,)); c.execute("DELETE FROM archives WHERE id=?", (arch_id,))
                                    commit_and_sync()
                                    st.session_state["flash_success"] = f"{nom} {prenom} réintégré(e). ✅"
                                    st.rerun()
                            else:
                                st.info("Un transféré ou un défunt ne peut pas être réintégré.")
        else:
